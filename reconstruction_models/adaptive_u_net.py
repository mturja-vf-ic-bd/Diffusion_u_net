...
class HeatDiffusionKernel(nn.Module):
    def __init__(self, in_feat, dim, n_modes, ks, n_class, act=nn.ELU(), drop_p=0.1, k=2, device="cpu"):
        ...
        self.female_proj = UNet.GraphUnet(ks=[0.5, 0.5], dim=1, act=act, drop_p=drop_p, k=k)
        self.female_out = nn.Linear(148, 148)
        self.male_proj = UNet.GraphUnet(ks=[0.5, 0.5], dim=1, act=act, drop_p=drop_p, k=k)
        self.male_out = nn.Linear(148, 148)
    ...
    def forward(self, phi_0, g, t, U, V, l, sx):
        x_0 = self.in_proj(phi_0)
        x_0 = self.init_depo(g, x_0)
        x_0 = self.out_proj(x_0[0][-1])
        x_male = self.male_out(x_0.squeeze()).unsqueeze(-1)
        x_female = self.female_out(x_0.squeeze()).unsqueeze(-1)
        x_both = torch.cat([x_female, x_male], dim=-1)
        x_0 = x_both.gather(-1, sx.long())
