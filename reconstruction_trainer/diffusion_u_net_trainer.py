...
from reconstruction_models.diffusion_u_net import HeatDiffusionKernel
...
def compute_loss(y_pred, y, x_0, diff_norm):
    loss_temp = torch.mul((y_pred - y) ** 2, (y != 0)).sum(dim=1).mean() * args.loss_w[0]
    loss_ent = -(torch.softmax(x_0, dim=1) * torch.log_softmax(x_0, dim=1)).sum(dim=1).mean() * args.loss_w[1]
    loss_abs = torch.abs(diff_norm).mean() * args.loss_w[2]
...
def process_function(engine, batch):
    ...
    optimizer.zero_grad()
    x, y, dx, g, t = batch
    y_pred, beta, x_0, diff_norm = model(x[:, :, 0:1], g, t, U, V, l)
    loss, recon_loss,  ent_loss, l1_loss = compute_loss(y_pred, y, x_0, diff_norm)
    loss.backward()
    optimizer.step()
    return {"prediction": y_pred, "true": y, "loss": loss.item(),
            "recon_loss": recon_loss.item(), "ent_loss": ent_loss.item(), "l1_loss": l1_loss}
...
if train_model:
    ...
    def score_function(engine):
        val_loss = evaluator.state.metrics['recon_loss']
        return -val_loss
