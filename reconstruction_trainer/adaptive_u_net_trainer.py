...
if load_model:
    if args.model_ckp_path == "saved_model":
        ...
        model = HeatDiffusionKernel(in_feat=2, dim=args.indim,
                                    n_modes=args.modes, act=nn.ELU(), ks=args.ks, n_class=len(set(label_all.values())),
    ...
    model = HeatDiffusionKernel(in_feat=2, dim=args.indim, n_class=len(set(label_all.values())),
...
def compute_loss(y_pred, y, x, dx, x_0, diff_norm):
    ...
    return loss_temp + loss_ent + loss_abs, loss_temp, loss_ent, loss_abs
...
def process_function(engine, batch):
    ...
    optimizer.zero_grad()
    x, y, dx, g, t = batch
    y_pred, beta, x_0, diff_norm = model(x[:, :, 0:2], g, t, U, V, l, x[:, :, 2:3])
    loss, recon_loss, ent_loss, l1_loss = compute_loss(y_pred, y, x, dx, x_0, diff_norm)
    loss.backward()
    optimizer.step()
    return {"prediction": y_pred, "true": y, "loss": loss.item(), "dx": dx,
                "recon_loss": recon_loss.item(), "ent_loss": ent_loss.item(),
                "l1_loss": l1_loss}
...
def print_logs(engine, dataloader, mode, history_dict):
    metrics = ["loss", "recon_loss", "ent_loss", "l1_loss"]
...
if train_model:
    ...
    metric_names = ["loss", "recon_loss", "ent_loss", "l1_loss"]
    training_history = {}
    validation_history = {}
    evaluation_history = {}
    ...
    def score_function(engine):
        val_loss = evaluator.state.output['recon_loss']
        return -val_loss
