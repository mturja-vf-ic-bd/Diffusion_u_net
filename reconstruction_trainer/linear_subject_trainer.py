...
from reconstruction_models.linear_subject import HeatDiffusionKernel
...
def compute_loss(y_pred, y):
    loss_temp = torch.mul((y_pred - y) ** 2, (y != 0)).sum(dim=1).mean()
    return loss_temp
...
if train_model:
    ...
    def score_function(engine):
        val_loss = evaluator.state.output['loss']
        return -val_loss
