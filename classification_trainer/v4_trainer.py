...
from classification_models.v4 import MinMaxClassifier
...
if load_model:
    ...
    # model = MinMaxClassifier(in_feat=2, dim=args.indim, n_class=len(set(label_all.values())),
    #                          act=nn.ELU(), cls_hdn=args.cls_hdn, cls_layer_count=args.n_cls_lyr,
    #                          k=args.n_neigh, drop_p=args.dropout).to(device)
    # model = load_checkpoint(model, os.path.join(model_log_dir, args.model_ckp_path))
else:
    model = MinMaxClassifier(in_feat=2, dim=args.indim, n_class=len(set(label_all.values())),
                             act=nn.ELU(), cls_hdn=args.cls_hdn, cls_layer_count=args.n_cls_lyr,
                             k=args.n_neigh, drop_p=args.dropout).to(device)
...
def compute_loss(y_pred, y, x, dx_pred, dx, x_0, diff_norm):
    dx = dx.squeeze().long()
    y = torch.cat([x[:, :, 0:1].transpose(1, 2), y], dim=1)
    ...
    return loss_temp + loss_cls + loss_ent + loss_abs, loss_temp, loss_cls, loss_ent, loss_abs
...
def process_function(engine, batch):
    ...
    optimizer.zero_grad()
    x, y, dx, g, t = batch
    final_cls_score = model(x[:, :, 0:1], y, g)
    loss = criterion_cls(final_cls_score, dx.squeeze().long())
    loss.backward()
    optimizer.step()
    return {"loss": loss.item(), "dx_pred": final_cls_score, "dx": dx}
...
def print_logs(engine, dataloader, mode, history_dict):
    ...
    print_line += " ,Micro: {:.4f} ".format(evaluator.state.metrics["micro"])
    print_line += " ,Accuracy: {:.4f} ".format(evaluator.state.metrics["Accuracy"])
    tb_log.add_scalar("{}/{}".format(mode, "Accuracy"), evaluator.state.metrics["Accuracy"], engine.state.epoch)
    for m in metrics:
...
if train_model:
    ...
    binary_accuracy.attach(evaluator, "Accuracy")
    binary_accuracy.attach(trainer, "Accuracy")
    metric_names = ["loss"]
    training_history = {}
    validation_history = {}
    evaluation_history = {}
    ...
    # trainer.add_event_handler(Events.EPOCH_COMPLETED, print_logs, eval_loader, 'Evaluation', evaluation_history)
...
with torch.no_grad():
    for batch in valid_loader:
        ...
        with open(os.path.join(FILE_PREFIX, "LearningInStructure/results", "gen_min_max.p"), "wb") as f:
            pickle.dump({"dx_pred": final_cls_score, "dx": dx}, f)
